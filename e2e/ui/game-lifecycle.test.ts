...
test.describe('Game Lifecycle', () => {
  ...
  test('creates and runs a complete game with mock models', async ({ request, page }) => {
    ...
    // Use /health endpoint since test games don't persist to D1
    let gameStatus = 'running';
    for (let i = 0; i < 60; i++) {
      await page.waitForTimeout(1000);
      const statusResponse = await request.get(`${WORKER_URL}/api/games/${gameId}/health`);
      if (statusResponse.ok()) {
        const healthData = await statusResponse.json();
        gameStatus = healthData.status || 'unknown';
  ...
  // NOTE: Test games using test/* models don't persist to D1 to avoid polluting
  // production data. This test verifies games list shows real games (not test games).
  test('game appears in games list after completion', async ({ page }) => {
    // Navigate to games list (shows real games from D1)
    await page.goto('/games');
    
    // The table should be visible
    await expect(page.locator('table')).toBeVisible({ timeout: 10000 });
    
    // Verify the table has some games (from previous production runs)
  ...
  test('can view game transcript after completion', async ({ request, page }) => {
    ...
    // 2. Wait for game completion using /health endpoint (test games skip D1)
    let completed = false;
    for (let i = 0; i < 30; i++) {
      await page.waitForTimeout(1000);
      const response = await request.get(`${WORKER_URL}/api/games/${gameId}/health`);
      if (response.ok()) {
    ...
    // 3. Verify transcript is available via API (R2 storage)
