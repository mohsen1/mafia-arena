...
import { ArrowLeft, Loader2, Feather, Scroll, Building2, Sparkles } from 'lucide-react';

// Theme configuration with icons and styling
...
export default function LiveGame({ loaderData }: Route.ComponentProps) {
  ...
  const themeKey = game.persona_theme || 'noir';
  const theme = THEME_CONFIG[themeKey] || THEME_CONFIG.noir;
  const participants = game.participants || [];
  const mafiaPlayers = participants.filter(p => p.team === 'mafia');
  const townPlayers = participants.filter(p => p.team === 'town');
  const mafiaModels = [...new Set(mafiaPlayers.map(p => p.model_name.replace(/-\d{4}-\d{2}-\d{2}$/, '').replace(/@.*$/, '')))];
  const townModels = [...new Set(townPlayers.map(p => p.model_name.replace(/-\d{4}-\d{2}-\d{2}$/, '').replace(/@.*$/, '')))];

  return (
    <div className="fixed inset-0 top-14 flex flex-col overflow-hidden bg-background" id="live-game-container" data-game-id={gameId} data-api-url={apiUrl}>
      ...
      <div className="shrink-0 px-4 pt-2 pb-1">
        ...
        <div className="flex items-center gap-3 text-[10px]">
          ...
          <div id="teams-display" className="flex items-center gap-1.5">
            <span className="text-rose-500 font-bold">MAFIA</span>
            <span id="mafia-models" className="font-mono text-foreground/60 truncate max-w-[120px]">
              {mafiaModels.join(', ') || '—'}
            </span>
            <span className="text-muted-foreground/40 text-[8px]">vs</span>
            <span className="text-indigo-500 font-bold">TOWN</span>
            <span id="town-models" className="font-mono text-foreground/60 truncate max-w-[120px]">
              {townModels.join(', ') || '—'}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 text-[9px] text-muted-foreground">
            <span>Round <span id="round-display" className="font-mono font-bold text-foreground">-</span></span>
            <div id="phase-display" className="inline-flex items-center gap-1 font-medium text-foreground">Starting</div>
            <span id="duration-display" className="font-mono tabular-nums">00:00</span>
            <span id="token-display" className="font-mono font-bold text-emerald-600 dark:text-emerald-400 tabular-nums">0</span>
          </div>
        </div>
