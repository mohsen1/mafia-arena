...
export async function advanceGameStateAction(gameId: string): Promise<FilteredGameState | { error: string }> {
    try {
        ...
        const filteredState = filterGameStateForClient(newState, newState.humanPlayerId);
        return filteredState;
    } catch (error) {
        console.error('Error advancing game state:', error);
        return { error: error instanceof Error ? error.message : 'Failed to advance game state' };
    }
}

export async function getGameStateAction(gameId: string): Promise<FilteredGameState | { error: string }> {
    try {
        // Check authentication
        const session = await getServerSession(authOptions);
        if (!session?.user?.id) {
        ...
        const filteredState = filterGameStateForClient(gameState, gameState.humanPlayerId);
        return filteredState;
    } catch (error) {
