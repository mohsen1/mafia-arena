...
export async function POST(request: NextRequest) {
  try {
    ...
    const { text, voice_id = DEFAULT_VOICE_ID, model_id = 'eleven_multilingual_v2', with_timestamps = false } = await request.json();
    console.log(`API Request - Text: "${text.substring(0, 50)}...", Voice: ${voice_id}, Model: ${model_id}, Timestamps: ${with_timestamps}`);
    ...
    headers.append('Accept', with_timestamps ? 'application/json' : 'audio/mpeg');
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorText = errorData.detail?.message || JSON.stringify(errorData.detail) || errorText;
      } catch (e) {
    ...
    if (with_timestamps) {
        ...
        console.log('Received JSON with audio_base64 and alignment from ElevenLabs.');
        ...
        if (!response.body) {
            console.error('Error: Response body from ElevenLabs streaming API is null');
            return NextResponse.json({ error: 'Received empty response body from upstream API' }, { status: 500 });
        }
        console.log('Received stream from ElevenLabs API. Returning NextResponse...');
    ...
    return NextResponse.json({ error: error.message || 'Failed to process speech request' }, { status: 500 });
  }
}
