...
import { getSystemPrompt, getUserPrompt, getPersonaGenerationPrompt } from '../prompts';
...
export class OpenAIAgent implements IAgent {
    ...
    constructor(id: PlayerId, model: string = DEFAULT_MODEL, apiBase: string = DEFAULT_API_BASE, apiKey: string | undefined = DEFAULT_API_KEY) {
        this.id = id;
        this.model = model;
        ...
        if (!apiKey) {
            log(`WARN: OpenAI Agent ${this.id}: API key for model ${this.model} at ${this.apiBase} is not set. Requests might fail.`);
        }
