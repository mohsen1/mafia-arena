...
export function GameInsights({ gameState, className }: GameInsightsProps) {
  ...
  const metrics = useMemo((): GameMetrics => {
    ...
    const eliminationRate = gameState.round > 0 
      ? deadPlayers / gameState.round 
      : 0;
    
    // Discussion intensity (messages per round)
    const totalMessages = gameState.log.filter(m => m.type === 'chat').length;
    const discussionIntensity = gameState.round > 0
      ? totalMessages / gameState.round
      : 0;
    ...
    const votingCoherence = currentVotes.length > 0
    ...
    // Predict winner based on current state
  ...
  const insights = useMemo((): GameInsight[] => {
    ...
    if (metrics.gamePace === 'fast') {
      insights.push({
        ...
        description: t('FastPaceDesc', 'Multiple eliminations per round - be careful with accusations'),
        priority: 'high'
      });
      ...
      insights.push({
        ...
        description: t('SlowPaceDesc', 'Players are being careful - look for subtle clues'),
        priority: 'medium'
      });
    }
    ...
    if (metrics.votingCoherence > 0.7 && gameState.phase === 'Day') {
      insights.push({
        ...
        description: t('ConsensusDesc', 'Most players agree on the target - consider if this is justified'),
        priority: 'high',
        actionable: humanPlayer?.status === 'Alive' 
          ? t('ConsiderJoining', 'Consider joining the majority or defending the target')
          : undefined
      });
    }
    ...
    if (metrics.discussionIntensity < 5 && gameState.round > 2) {
      insights.push({
        ...
        description: t('LowDiscussionDesc', 'Quiet players might be hiding something'),
        priority: 'medium',
        actionable: humanPlayer?.status === 'Alive'
    ...
    if (metrics.winProbability > 70) {
      insights.push({
        ...
        description: t('WinProbabilityDesc', '{{probability}}% chance based on current numbers', {
    ...
    if (humanPlayer?.status === 'Alive') {
      if (humanPlayer.role === 'Seer' && gameState.phase === 'Night') {
        insights.push({
          ...
          description: t('SeerStrategyDesc', 'Focus on quiet or defensive players'),
          priority: 'high',
        ...
        insights.push({
          ...
          description: t('DoctorStrategyDesc', 'Protect active town members or yourself'),
          priority: 'high',
          actionable: t('ProtectActive', 'Consider protecting the most vocal player')
        });
      }
    }
  ...
  return (
    <Card className={cn("overflow-hidden", className)}>
      ...
      <CardContent className="space-y-3 p-3">
        ...
        {metrics.predictedWinner !== 'uncertain' && (
          <div className="p-3 rounded-lg bg-accent/50">
            <div className="flex items-center justify-between mb-2">
              ...
              <Badge 
                variant={metrics.predictedWinner === 'town' ? 'default' : 'destructive'}
                className="text-xs"
              >
            ...
            <Progress 
              ...
              className={cn(
                "h-2",
                metrics.predictedWinner === 'town' ? "bg-blue-200" : "bg-red-200"
              )}
            />
        ...
        <div className="grid grid-cols-3 gap-2 pt-3 border-t">
          <div className="text-center">
            <p className="text-xs text-muted-foreground">{t('GamePace', 'Pace')}</p>
            <p className="text-sm font-medium capitalize">{t(metrics.gamePace)}</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-muted-foreground">{t('Discussion', 'Discussion')}</p>
            <p className="text-sm font-medium">
              {metrics.discussionIntensity > 10 ? t('High') : 
               metrics.discussionIntensity > 5 ? t('Medium') : t('Low')}
            </p>
          </div>
          <div className="text-center">
            <p className="text-xs text-muted-foreground">{t('Unity', 'Unity')}</p>
            <p className="text-sm font-medium">
