...
export function GameStatsTracker({ gameState, className }: GameStatsTrackerProps) {
  ...
  const stats = useMemo(() => {
    ...
    const survivalRate = totalPlayers > 0 ? (alivePlayers / totalPlayers) * 100 : 0;

    // Count votes in current round
    const votesThisRound = gameState.log.filter(
      msg => msg.round === gameState.round && 
             msg.content.includes('votes for')
    ).length;

    // Calculate game pace (messages per round)
    const avgMessagesPerRound = gameState.round > 0 
      ? Math.round(gameState.log.filter(m => m.type === 'chat').length / gameState.round)
      : 0;

    // Detect if mafia is winning
    const mafiaCount = Object.values(gameState.players).filter(
      p => p.isMafia && p.status === 'Alive'
    ).length;
    const townCount = alivePlayers - mafiaCount;
    const mafiaWinning = mafiaCount >= townCount;
    ...
    const gameStats: GameStat[] = [
      {
        ...
        description: t('Stats.SurvivalDesc', '{{alive}} of {{total}} players alive', {
          alive: alivePlayers,
          total: totalPlayers
        })
      },
      ...
      },
      {
        ...
        value: mafiaWinning ? t('MafiaLeading', 'Mafia') : t('TownLeading', 'Town'),
        icon: <Activity className="w-4 h-4" />,
        trend: mafiaWinning ? 'down' : 'up',
        color: mafiaWinning ? 'text-red-500' : 'text-green-500',
        description: t('Stats.BalanceDesc', '{{town}} town vs {{mafia}} mafia', {
          town: townCount,
          mafia: mafiaCount
        })
      }
  ...
  const gameProgress = useMemo(() => {
    ...
    const progressPercentage = (deadPlayers / Math.max(totalPlayers - 1, 1)) * 100;
                ...
                <div className="flex items-start justify-between mb-1">
                  ...
                  {stat.trend && (
                    <div className={cn("text-xs", {
                      ...
                      {stat.trend === 'neutral' && <Activity className="w-3 h-3" />}
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-1">
              {gameProgress > 60 && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>{t('GameNearingEnd', 'Game is nearing its conclusion')}</span>
                </div>
              )}
              {messagesThisRound > 10 && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <MessageSquare className="w-3 h-3" />
                  <span>{t('HighActivity', 'High discussion activity this round')}</span>
                </div>
              )}
            </div>
          </div>
