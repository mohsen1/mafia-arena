...
import { supportedLanguages, SupportedLanguage } from "@/hooks/useGameConfig";
...
export default function StartGameForm({ availableModels, translations }: StartGameFormProps) {
    // Instantiate the translation hook here
    const { t, isLoading: isTLoading, error: tError } = useTranslation({
    ...
    // Handle translation error locally
     if (tError) {
         ...
         return <div className="text-red-500 p-4">Error with translations: {tError}</div>;
     }
    ...
    const numberFormatter = useMemo(() => {
        ...
        const locale = langCode === 'fa' ? 'fa-IR' : (langCode === 'de' ? 'de-DE' : 'en-US'); // Map to appropriate locale
        try {
    ...
    return (
        <div className="mb-8 p-6 bg-white dark:bg-gray-800 shadow-md rounded-lg border border-gray-200 dark:border-gray-700 relative">
            ...
            <h2 className="text-2xl font-bold mb-6 text-gray-700 dark:text-gray-300 text-center">{t('ConfigureNewGameTitle', 'Configure New Game')}</h2>

            {/* Player Count Adjustment - Use Formatter */}
             <div className="mb-4 flex items-center justify-center gap-4">
                 <Label className="text-sm font-medium text-gray-600 dark:text-gray-400">{t('PlayersLabel', 'Players')}:</Label>
                 {/* Format the totalSlots number */}
                 <span className="text-lg font-semibold text-gray-800 dark:text-gray-200 w-10 text-center">{numberFormatter.format(totalSlots)}</span>
                 <Button
                      type="button" variant="outline" size="icon"
                      onClick={addPlayerSlot} disabled={isLoading}
             ...
             <div className="mb-6 flex items-center justify-center gap-2 rtl:flex-row-reverse">
                 ...
                 >
                    <SelectTrigger id="global-model-select" className="w-full max-w-xs text-sm h-9">
                        <SelectValue placeholder={t('SelectGlobalModelPlaceholder', "Select global model")} />
                    </SelectTrigger>
                    <SelectContent>
                        ...
                        {availableModels.length === 0 && initialSlotsSet && (
                             <SelectItem value="no-models" disabled>{t('NoModelsAvailableLabel', 'No models available')}</SelectItem>
                        )}
                    </SelectContent>
                 </Select>
            </div>
            ...
            <div className="mb-6 flex items-center justify-center gap-2 rtl:flex-row-reverse">
                 <Label htmlFor="language-select" className="text-sm font-medium text-gray-600 dark:text-gray-400 whitespace-nowrap">
                 ...
                 <Select
                    ...
                    disabled={isLoading}
                 >
                    <SelectTrigger id="language-select" className="w-full max-w-xs text-sm h-9">
            ...
            <div className="mb-4 p-4 border border-gray-200 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-750 min-h-[200px]">
                 <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-3 text-center flex items-center justify-center gap-2"><Settings2 className="h-5 w-5"/> {t('CharacterSetupLabel', 'Character Setup')}</h3>
                 {!initialSlotsSet && availableModels.length > 0 && (
                      <div className="flex justify-center items-center h-20 text-gray-500 dark:text-gray-400">
                          <Loader2 className="h-5 w-5 animate-spin mr-2" /> {t('LoadingSetupLabel', 'Loading setup...')}
                      </div>
                 )}
                 {availableModels.length === 0 && !initialSlotsSet && (
                    <p className="text-center text-sm text-yellow-600 dark:text-yellow-500">{t('WaitingForModelsLabel', 'Waiting for available AI models...')}</p>
                 )}
                 ...
                 {initialSlotsSet && characterSlots.length > 0 && (
                  ...
                  {initialSlotsSet && characterSlots.length === 0 && (
                      <p className="text-center text-sm text-gray-500 dark:text-gray-400 italic py-4">{t('AddPlayerSlotsPrompt', "Use the '+' button to add player slots (minimum 5).")}</p>
                  )}
            </div>
             ...
             <div className="h-10 text-center flex items-center justify-center px-2 mt-4 mb-2 text-sm">
                     {errorMsg ? (
                      <p className="text-red-600 dark:text-red-400 flex items-center gap-1"><AlertTriangle className="h-4 w-4"/> {t(errorMsg, errorMsg)}</p> // Attempt to translate error, fallback to original
                  ) : isSubmitting ? (
                      <p className="text-blue-600 dark:text-blue-400 flex items-center gap-1"><Loader2 className="h-4 w-4 animate-spin"/> {t(infoMsg || 'ProcessingLabel', infoMsg || 'Processing...')}</p> // Translate info message
                  ) : postGenValidationMsg ? (
                       <p className={`flex items-center gap-1 ${isPostGenValid === true ? 'text-green-600 dark:text-green-400' : (isPostGenValid === false ? 'text-yellow-600 dark:text-yellow-400' : 'text-gray-500 dark:text-gray-400')}`}>
                           ...
                           {t(postGenValidationMsg, postGenValidationMsg)} {/* Translate post-gen validation msg */}
                       </p>
                  ) : configValidation.isValid ? (
                      <p className="text-green-600 dark:text-green-400 flex items-center gap-1"><CheckCircle2 className="h-4 w-4"/> {t('ConfigLooksGood_Prefix', 'Configuration looks good') + ` (${numberFormatter.format(totalSlots)} ${t('PlayersCount_Suffix', 'players')}).`}</p>
                  ) : initialSlotsSet ? (
                      <p className="text-yellow-600 dark:text-yellow-400 flex items-center gap-1"><AlertTriangle className="h-4 w-4"/> {t('ConfigInvalid', configValidation.message || '')}</p>
                  ) : (
                       <p className="text-gray-500 dark:text-gray-400 italic">{t('InitialConfigPrompt', 'Configure player slots, roles, and models.')}</p>
                  )}
                </div>
            ...
            {/* Generate & Start Game Button */}
                ...
                aria-label={t('GenerateAndStartGameButton', "Generate characters and start new game")}
                 >
                     {isSubmitting ? (
                         <>
