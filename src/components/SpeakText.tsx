...
const SpeakText = React.memo<SpeakTextProps>(
  ({
    ...
    const _log = console.log.bind(console, '[SpeakText]');
  ...
  const _crossfadeTimeRef = useRef(150); // ms overlap for smooth transitions
    ...
    useEffect(() => {
      ...
      return () => {
        win.__audioElementCount = Math.max(
          0,
          (win.__audioElementCount || 0) - 1
        );
        console.log(
    ...
    const handleSpeak = useCallback(
      async (retryCount = 0) => {
        ...
        addAudioBreadcrumb('Permission granted', {
          audioId: audioIdRef.current,
        });
        ...
        try {
          ...
          eventHandlers.ended = () => {
            console.log(
              `[SpeakText] ${timestamp()} 🏁 Audio ENDED naturally:`,
              {
                audioId: audioIdRef.current,
          ...
          try {
            ...
            if (playError.name === 'NotAllowedError') {
              console.log(
                '[SpeakText] Audio blocked by browser autoplay policy'
              );
            }
          }
        } catch (error: any) {
    ...
    const cleanupAudio = useCallback(() => {
      if (audioRef.current) {
        const audio = audioRef.current;
        console.log(
          `[SpeakText] ${timestamp()} 🧹 Cleaning up audio element:`,
          {
            audioId: audioIdRef.current,
            src: audio.src?.substring(0, 50) + '...',
          }
    ...
    useEffect(() => {
      ...
      if (
        ...
        console.log(
          `[SpeakText] ${timestamp()} ⏭️ SKIPPING - Already started:`,
          {
            audioId: audioIdRef.current,
            textPreview: text.substring(0, 30) + '...',
          }
