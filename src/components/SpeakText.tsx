...
const measurePerformance = async <T,>(name: string, fn: () => T | Promise<T>): Promise<T> => {
...
const logAudioDiagnostics = () => {
  const diagnostics = {
    ...
    audioContextSupport: 'AudioContext' in window || 'webkitAudioContext' in window,
  ...
  console.log('%c[SpeakText] 🔍 AUDIO DIAGNOSTICS:', 'color: #f39c12; font-weight: bold', diagnostics);
  return diagnostics;
};
...
if (typeof window !== 'undefined' && !(window as any).__audioDiagnosticsLogged) {
...
if (typeof window !== 'undefined' && !(window as any).__audioNetworkMonitor) {
  ...
  window.addEventListener('online', () => {
    console.log('%c[SpeakText] 🌐 NETWORK ONLINE', 'color: #27ae60; font-weight: bold');
    audioMetrics.networkEvents = (audioMetrics.networkEvents || 0) + 1;
  });
  
  window.addEventListener('offline', () => {
    console.log('%c[SpeakText] 🚫 NETWORK OFFLINE', 'color: #e74c3c; font-weight: bold');
    audioMetrics.networkEvents = (audioMetrics.networkEvents || 0) + 1;
  });
  ...
  if ('connection' in navigator) {
    ...
    connection.addEventListener('change', () => {
      console.log('%c[SpeakText] 📶 CONNECTION CHANGED:', 'color: #f39c12; font-weight: bold', {
  ...
  window.onerror = function(message, source, lineno, colno, error) {
    if (String(message).toLowerCase().includes('audio') || 
        String(source).includes('SpeakText') ||
        (error && error.stack && error.stack.includes('audio'))) {
      console.error('%c[SpeakText] 🚨 UNHANDLED AUDIO ERROR:', 'color: #e74c3c; font-weight: bold', {
        message,
        source,
        lineno,
        colno,
...
export function SpeakText({
    ...
    const handleVisibilityChange = () => {
      ...
      if (audioRef.current && isPlaying) {
        if (isHidden) {
          ...
          console.log(`[SpeakText] ${timestamp()} ⏸️ PAUSING due to tab hidden`);
          ...
          console.log(`[SpeakText] ${timestamp()} ▶️ RESUMING after tab visible`);
          wasPlayingBeforeHiddenRef.current = false;
          audioRef.current.play().catch(err => {
            console.error(`[SpeakText] ${timestamp()} ❌ Error resuming after visibility:`, err);
          });
  ...
  useEffect(() => {
    activeAudioElements++;
    audioMetrics.audioElementPeakCount = Math.max(audioMetrics.audioElementPeakCount, activeAudioElements);
    console.log(`[SpeakText] ${timestamp()} 📊 AUDIO ELEMENT COUNT: ${activeAudioElements}`);
  ...
  useEffect(() => {
    ...
    if (!(window as any).__audioElementCount) (window as any).__audioElementCount = 0;
    (window as any).__audioElementCount++;
    console.log(`[SpeakText] ${timestamp()} 📊 AUDIO ELEMENT COUNT: ${(window as any).__audioElementCount}`);
    
    // Detect React StrictMode double rendering
    renderCountRef.current++;
    if (renderCountRef.current > 1) {
      console.log(`[SpeakText] ${timestamp()} ⚠️ STRICT MODE: Component rendered ${renderCountRef.current} times`);
    }
    
    return () => {
      (window as any).__audioElementCount = Math.max(0, ((window as any).__audioElementCount || 0) - 1);
      console.log(`[SpeakText] ${timestamp()} 📊 AUDIO ELEMENT COUNT (after cleanup): ${(window as any).__audioElementCount}`);
    };
  }, []);
  ...
  const fetchAudioWithDeduplication = async (url: string, signal: AbortSignal): Promise<Response> => {
  ...
  const handleSpeak = useCallback(async (retryCount = 0) => {
    ...
    if (text.length > 3000) {
      console.warn(`[SpeakText] ${timestamp()} ⚠️ VERY LONG MESSAGE - May exceed TTS limits`, {
        audioId: audioIdRef.current,
        length: text.length,
    ...
    console.log(`[SpeakText] ${timestamp()} 🗣️ handleSpeak CALLED:`, {
      ...
      caller: callerLine.includes('autoPlay') ? 'autoPlay-effect' : 
             callerLine.includes('Button') ? 'manual-button' : 'other',
      hasStartedRef: hasStartedRef.current,
      retryCount,
    });
    
    // Track manual vs auto play
    const caller = callerLine.includes('autoPlay') ? 'autoPlay-effect' : 
                   callerLine.includes('Button') ? 'manual-button' : 'other';
    ...
    if (audioRef.current && !audioRef.current.paused) {
      console.log(`[SpeakText] ${timestamp()} ⚠️ Audio already playing, ignoring duplicate call`);
      return;
    }

    // Prevent concurrent handleSpeak calls
    if (isHandlingSpeakRef.current) {
      console.log(`[SpeakText] ${timestamp()} ⚠️ Already handling speak, ignoring duplicate call`);
      return;
    }

    if (!isMountedRef.current) {
      console.log(`[SpeakText] ${timestamp()} ❌ Component unmounted, aborting`);
      return;
    }
    ...
    addAudioBreadcrumb('Request permission', { audioId: audioIdRef.current, caller });
    const result = requestPermissionToSpeak(audioIdRef.current);
    const granted = result.granted;

    if (!granted) {
      console.log(`[SpeakText] ${timestamp()} ❌ PERMISSION DENIED - another audio is playing`);
      addAudioBreadcrumb('Permission denied', { audioId: audioIdRef.current });
    ...
    try {
      ...
      if (audioRef.current) {
        console.log(`[SpeakText] ${timestamp()} ⚠️ Audio element already exists, aborting fetch`);
        isHandlingSpeakRef.current = false;
        return;
      }

      console.log(`[SpeakText] ${timestamp()} 🌐 Fetching audio from /api/speak:`, {
        audioId: audioIdRef.current,
      ...
      if (!isMountedRef.current) {
        console.log('[SpeakText] Component unmounted during fetch, cleaning up');
      ...
      eventHandlers.ended = () => {
        ...
        console.log(`[SpeakText] ${timestamp()} 🧹 Cleaning up audio after ended`);
      ...
      if (retryCount < 3 && (error.name === 'NetworkError' || error.message.includes('fetch'))) {
        console.log(`[SpeakText] ${timestamp()} 🔄 Retrying after network error (attempt ${retryCount + 1}/3)`);
        setTimeout(() => {
          if (isMountedRef.current) {
      ...
      performance.measure(`audio-speak-error-${audioIdRef.current}`, `${perfMark}-start`, `${perfMark}-error`);
    }
    
    // Performance mark end on success
    if (performance.getEntriesByName(`${perfMark}-start`).length > 0) {
      performance.mark(`${perfMark}-end`);
      performance.measure(`audio-speak-complete-${audioIdRef.current}`, `${perfMark}-start`, `${perfMark}-end`);
    }
  }, [text, voiceId, currentlySpeakingId, requestPermissionToSpeak, doneSpeaking, markAsPlaying]);
  ...
  useEffect(() => {
    if (audioRef.current && currentlySpeakingId !== audioIdRef.current) {
      console.log(`[SpeakText] ${timestamp()} 🛑 Audio stopped externally (Skip button):`, {
        audioId: audioIdRef.current,
        currentlySpeakingId,
      });
      ...
      if (audioRef.current && !audioRef.current.paused) {
        ...
        console.log(`[SpeakText] ${timestamp()} 📊 Audio interrupted:`, {
          ...
          completionRate: ((playedDuration / audioRef.current.duration) * 100).toFixed(1) + '%',
        });
      }
    ...
    const metricsInterval = setInterval(() => {
      ...
      log("📊", "AUDIO METRICS SUMMARY", {
        ...
        completionRate: totalEnded > 0
          ...
          : 'N/A',
        skipRate: totalEnded > 0
          ? `${((audioMetrics.interruptCount / totalEnded) * 100).toFixed(1)}%`
          : 'N/A',
        avgListenTime: audioMetrics.completionCount > 0
