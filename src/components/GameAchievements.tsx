...
export function GameAchievements({ gameState, className }: GameAchievementsProps) {
    ...
    return ACHIEVEMENT_DEFINITIONS.map(def => {
      ...
      switch (def.id) {
        case 'first_blood':
          ...
          const firstVote = gameState.log.find(
            msg => msg.content.includes('votes for')
          );
          unlocked = firstVote?.senderId === gameState.humanPlayerId;
          break;

        case 'survivor':
          unlocked = gameState.phase === 'GameOver' && 
        ...
        case 'guardian_angel':
          if (humanPlayer.role === 'Doctor') {
            ...
            unlocked = gameState.log.some(
              msg => msg.content.includes('was saved by the Doctor')
            );
          }
          break;
        ...
        case 'mastermind':
          if (humanPlayer.role === 'Mafia' && gameState.phase === 'GameOver') {
            ...
            const mafiaWon = gameState.log.some(
              msg => msg.content.includes('Mafia wins')
            );
        ...
        case 'social_butterfly':
          const messageCount = gameState.log.filter(
            msg => msg.senderId === gameState.humanPlayerId && msg.type === 'chat'
          ).length;
        ...
        case 'consensus_builder':
          ...
          const humanVotes = gameState.log.filter(
            msg => msg.senderId === gameState.humanPlayerId && 
        ...
        case 'speed_demon':
          if (gameState.phase === 'GameOver' && humanPlayer.status === 'Alive') {
        ...
        case 'underdog':
          if (gameState.phase === 'GameOver' && humanPlayer.status === 'Alive' && 
            ...
            const underworldScenario = finalLog.some(
              msg => msg.content.includes('1 Town vs 2 Mafia')
            );
            const townWon = gameState.log.some(
              msg => msg.content.includes('Town wins')
            );
                      ...
                      <TooltipTrigger asChild>
                        <motion.div
                          className={cn(
                            ...
                            newAchievements.includes(achievement.id) && "ring-2 ring-primary"
                          )}
                      ...
                      <TooltipContent>
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            ...
                            {achievement.unlocked && (
                              <span className="text-green-500">✓ {t('Unlocked')}</span>
                            )}
                          </div>
                          {achievement.maxProgress && (
                            <p className="text-xs">
                              {t('Progress')}: {achievement.progress || 0}/{achievement.maxProgress}
                            </p>
                          )}
                        </div>
                      </TooltipContent>
