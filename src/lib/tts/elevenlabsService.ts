...
if (!apiKey) {
    console.warn("Missing ELEVENLABS_API_KEY environment variable. TTS features will be disabled.");
}
...
export async function generateAndSaveAudio(
    ...
    try {
        console.log(`Generating audio for message ${messageId} using voice ${voiceId}...`);
        ...
        console.error(`Failed to generate or save audio for message ${messageId}:`, error);
        return null;
    }
} 