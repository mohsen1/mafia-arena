...
vi.mock('node:fs/promises', async (importOriginal) => {
  ...
  return {
    ...
    default: {
      // Ensure default export is preserved/mocked
...
describe('Persistence Functions', () => {
  ...
  const mockGameState: Partial<SerializableGameState> = {
    gameId: gameId,
    round: 1,
    phase: 'Day',
  };
  ...
  describe('loadGameData', () => {
    ...
    it('should throw an error if reading file fails for other reasons', async () => {
      ...
      await expect(loadGameData(gameId)).rejects.toThrow(
        'Failed to load game data: Read permission denied'
      );
      expect(mockedMkdir).toHaveBeenCalled();
      expect(mockedReadFile).toHaveBeenCalledWith(filePath, 'utf-8');
    });
    ...
    it('should throw an error for invalid gameId', async () => {
      await expect(loadGameData('../invalid-id')).rejects.toThrow(
        'Invalid gameId'
      );
    });
  });
  ...
  describe('saveGameData', () => {
    ...
    it('should throw an error if writing file fails', async () => {
      ...
      await expect(saveGameData(gameId, stateToSave)).rejects.toThrow(
        'Failed to save game data: Disk full'
      );
      expect(mockedMkdir).toHaveBeenCalled();
    ...
    it('should throw an error for invalid gameId', async () => {
      const stateToSave = mockGameState as SerializableGameState;
      await expect(saveGameData('/invalid-id', stateToSave)).rejects.toThrow(
        'Invalid gameId'
      );
    });
  });
  ...
  describe('deleteGameData', () => {
    ...
    it('should throw an error if deleting file fails for other reasons', async () => {
      ...
      await expect(deleteGameData(gameId)).rejects.toThrow(
        'Failed to delete game data: Permission denied'
      );
      expect(mockedMkdir).toHaveBeenCalled();
      expect(mockedUnlink).toHaveBeenCalledWith(filePath);
    });
    ...
    it('should throw an error for invalid gameId', async () => {
      await expect(deleteGameData('invalid/../../id')).rejects.toThrow(
        'Invalid gameId'
      );
    });
  });
  ...
  describe('listSavedGames', () => {
    ...
    it('should throw an error if reading directory fails', async () => {
      ...
      await expect(listSavedGames()).rejects.toThrow(
        'Failed to list saved games: Directory access denied'
      );
      expect(mockedMkdir).toHaveBeenCalled();
      expect(mockedReaddir).toHaveBeenCalledWith(SAVE_DIR);
    });
