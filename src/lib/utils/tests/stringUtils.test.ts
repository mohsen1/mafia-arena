...
describe('stringUtils', () => {
  describe('cleanAIResponse', () => {
    it('should remove <think> blocks', () => {
      const response =
        'This is the result. <think>I should consider edge cases.</think> More result.';
      expect(cleanAIResponse(response)).toBe(
        'This is the result.  More result.'
      );
    });
    ...
    it('should remove common reasoning prefixes', () => {
      expect(cleanAIResponse('Reasoning: It must be player 2.')).toBe(
        'It must be player 2.'
      );
      expect(cleanAIResponse('Thought: Player 1 seems suspicious.')).toBe(
        'Player 1 seems suspicious.'
      );
      expect(cleanAIResponse("Thinking: Okay, let's vote.")).toBe(
        "Okay, let's vote."
      );
      expect(cleanAIResponse('Analysis: Based on the votes...')).toBe(
        'Based on the votes...'
      );
    });
    ...
    it('should handle combined cleaning cases', () => {
      const response =
        '  Reasoning: The data <think>might be wrong</think> shows `p1`. ```json\n{"action": "vote"}\n``` \n\n\n Final thought. ';
      expect(cleanAIResponse(response)).toBe(
        'The data  shows p1. \n{"action": "vote"}\n \n\n Final thought.'
      );
    });
  ...
  describe('escapeJSONControlCharacters', () => {
    ...
    it('should handle empty strings and return empty', () => {
      ...
      expect(escapeJSONControlCharacters('{"empty": ""}')).toBe(
        '{"empty": ""}'
      );
    });
