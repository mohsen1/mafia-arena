...
export class GameStateManager {
  ...
  async createAndSaveGame(gameState: GameState): Promise<GameState> {
    ...
    if (
      this.#gameStates.has(gameId) ||
      (await this.#loadGameStateFromFile(gameId)) !== null
    ) {
    ...
    console.log(
      `Game created and saved with ID: ${gameId} and Title: "${gameState.title}"`,
    );
    return gameState;
  }
  ...
  async updateGameState(
    ...
    if (
      !this.#gameStates.has(gameId) &&
      !(await this.#loadGameStateFromFile(gameId))
    ) {
      throw new Error(
        `Game with ID ${gameId} not found and cannot be updated.`,
      );
    }
   ...
   */
  async getFilteredGameState(
    gameId: string,
  ): Promise<FilteredGameState | null> {
