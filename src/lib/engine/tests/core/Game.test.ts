...
describe('Game', () => {
  ...
  const getAgentMock = (playerId: PlayerId) => {
    ...
    if (!player)
      throw new Error(
        `Player ${playerId} not found in test setup (getAgentMock)`
      );
  ...
  describe('Game initialization', () => {
    ...
    it('should throw error if fewer than 3 players are provided', () => {
      ...
      expect(() => Game.createNewGame(fewPlayerSetups)).toThrow(
        'Not enough players to start a new game (minimum 3).'
      );
    });
  });
  ...
  describe('Player Accessors', () => {
    ...
    it('should get alive Mafia', () => {
      ...
      expect(aliveMafia.map((p) => p.id)).toEqual(
        expect.arrayContaining([mafia1Id, mafia2Id])
      );
    ...
    it('should get alive Villagers', () => {
      ...
      // Check using actual IDs
      expect(aliveVillagers.map((p) => p.id)).toEqual(
        expect.arrayContaining([villager1Id, villager2Id, villager3Id])
      );
      ...
      expect(aliveVillagers.map((p) => p.id)).toEqual(
        expect.arrayContaining([villager2Id, villager3Id])
      );
    });
  ...
  describe('Game Flow (Simplified - testing state transitions indirectly)', () => {
    it.skip('should eventually reach GameOverPhase when win condition is met', async () => {
      ...
      mafiaAgent1.mockResolvedValue({
        type: 'mafiaKill',
        targetPlayerId: villager1Id,
      });
      mafiaAgent2.mockResolvedValue({
        type: 'mafiaKill',
        targetPlayerId: villager1Id,
      });
      // Day 1: V2, V3, M2 vote for M1 (M1 executed) -> Alive: V2, V3, M2
      // Assuming majority of 4 living players = 3 votes needed
      villagerAgent2.mockResolvedValue({
        type: 'vote',
        targetPlayerId: mafia1Id,
      });
      villagerAgent3.mockResolvedValue({
        type: 'vote',
        targetPlayerId: mafia1Id,
      });
      ...
      mafiaAgent2.mockResolvedValue({
        type: 'mafiaKill',
        targetPlayerId: villager2Id,
      });
      // Day 2: (Game likely ends here as Mafia >= Town)
      villagerAgent3.mockResolvedValue({
        type: 'vote',
        targetPlayerId: mafia2Id,
      });
      ...
      expect(mockRenderer.renderGameOver).toHaveBeenCalledWith(
        'Mafia',
        expect.anything()
      );
    });
  ...
  describe('Killing Players (killPlayer)', () => {
    ...
    it('should notify renderers about the status update', () => {
      ...
      const publicInfoAfterKill = game
        .getPlayer(playerIdToKill)
  ...
  describe('Agent Memory Handling', () => {
    ...
    it('should record Seer results only in the Seer agent memory', () => {
      ...
      const seerId = Array.from(game.getPlayers().values()).find(
        (p) => p.role.name === RoleName.Seer
      )!.id;
      const mafiaId = Array.from(game.getPlayers().values()).find(
        (p) => p.role.name === RoleName.Mafia
      )!.id;
      const villagerId = Array.from(game.getPlayers().values()).find(
        (p) => p.role.name === RoleName.Villager
      )!.id;
    ...
    it('should record Doctor save results only in the Doctor agent memory', () => {
      ...
      game = Game.createNewGame(setups);
      const doctorId = Array.from(game.getPlayers().values()).find(
        (p) => p.role.name === RoleName.Doctor
      )!.id;
      const mafiaId = Array.from(game.getPlayers().values()).find(
        (p) => p.role.name === RoleName.Mafia
      )!.id;
    ...
    it('should include relevant message history in agent memory', () => {
      ...
      expect(v1State.memory.messageHistory.map((m) => m.content)).toEqual([
        'Hello public',
        'System message',
      ]);
      ...
      expect(m1State.memory.messageHistory.map((m) => m.content)).toEqual([
        'Hello public',
        'Mafia secret chat',
        'System message',
      ]);
  ...
  describe('Logging Messages', () => {
    ...
    it('logMessage should notify renderers for Public messages', () => {
      ...
      expect(mockRenderer.renderMessage).toHaveBeenCalledWith(
        expect.objectContaining({ content: 'Public Msg' })
      );
    });
    ...
    it('logMessage should notify renderers for Mafia messages', () => {
      ...
      expect(mockRenderer.renderMessage).toHaveBeenCalledTimes(1);
      expect(mockRenderer.renderMessage).toHaveBeenCalledWith(
        expect.objectContaining({ content: 'Mafia Msg' })
      );
    });
    ...
    it('killPlayer should log a public message and notify renderers', () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        ...
        expect.stringContaining(
          `${player.name} (${player.role.name}) was voted out`
        ),
        MessageVisibility.Public
      );
  ...
  describe('Night Phase Actions (via Game.requestPlayerAction)', () => {
    ...
    let doctorId: PlayerId,
      seerId: PlayerId,
      nightMafiaId1: PlayerId,
      nightMafiaId2: PlayerId,
      nightVillagerId: PlayerId;
    ...
    it('should only request actions from players with night actions when game.requestPlayerAction is called by a phase', async () => {
      const requestActionSpy = vi.spyOn(
        nightTestGame,
        'requestPlayerAction'
      ) as MockedFunction<typeof nightTestGame.requestPlayerAction>;
      ...
      await nightTestGame.requestPlayerAction(mafia1Player, [
        'message',
        'noAction',
      ]);
      await nightTestGame.requestPlayerAction(mafia2Player, [
        'message',
        'noAction',
      ]);
      await nightTestGame.requestPlayerAction(mafia1Player, [
      ...
      await nightTestGame.requestPlayerAction(doctorPlayer, [
        'doctorSave',
        'noAction',
      ]);
      await nightTestGame.requestPlayerAction(seerPlayer, [
        'seerInvestigate',
        'noAction',
      ]);
  ...
  describe('runGameLoop', () => {
    let mockPhase: IGamePhase;
    let transitionSpy: MockedFunction<(game: Game) => GamePhaseType>;

    beforeEach(() => {
      // Create a mock phase that can be controlled
      transitionSpy = vi.fn().mockReturnValue('GameOver');
      mockPhase = {
        type: 'Day' as GamePhaseType,
        runStep: vi.fn().mockResolvedValue(undefined),
        transition: transitionSpy,
      };

      // Replace the current phase with our mock
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (game as any)['#currentState'] = mockPhase;
    });

    it('should run basic game loop until GameOver', async () => {
      // Setup: Mock checkWinCondition to return a winner after 2 iterations
      const checkWinSpy = vi.spyOn(game, 'checkWinCondition');
      checkWinSpy
        .mockReturnValueOnce(null) // First check: no winner
        .mockReturnValueOnce('Town'); // Second check: Town wins
      ...
      // Verify the loop ran and ended correctly
    ...
    it('should pause game loop when human action is required', async () => {
      // Setup: Mock pending human action
      const pendingAction = {
        playerId: villager1Id,
        allowedActions: ['vote'] as PlayerAction['type'][],
      ...
      // Verify the loop paused for human action
      expect(mockPhase.runStep).toHaveBeenCalledTimes(1);
      expect(checkWinSpy).toHaveBeenCalledTimes(1);
      expect(game.getCurrentPhaseType()).not.toBe('GameOver');
    });

    it('should handle infinite loop protection', async () => {
      // Setup: Mock a stuck phase that never progresses
      const stuckPhase = {
        type: 'Day' as GamePhaseType,
        runStep: vi.fn().mockResolvedValue(undefined),
        transition: vi.fn().mockReturnValue('Day'), // Always return same phase
      };
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    it('should handle phase execution errors gracefully', async () => {
      // Setup: Mock phase that throws an error
      const errorPhase = {
        type: 'Day' as GamePhaseType,
        runStep: vi.fn().mockRejectedValue(new Error('Phase execution failed')),
        transition: vi.fn().mockReturnValue('GameOver'),
      };
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (game as any)['#currentState'] = errorPhase;
      ...
      expect(logEventSpy).toHaveBeenCalledWith(
        'Critical error during Day phase step. Game cannot safely continue.'
      );
      expect(advancePhaseSpy).toHaveBeenCalledWith('GameOver', undefined);
    });

    it('should handle CharacterGeneration phase specially', async () => {
      // Setup: Mock CharacterGeneration phase
      const charGenPhase = {
        type: 'CharacterGeneration' as GamePhaseType,
        runStep: vi.fn().mockResolvedValue(undefined),
        transition: vi.fn().mockReturnValue('Day'),
      };
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (game as any)['#currentState'] = charGenPhase;

      const checkWinSpy = vi.spyOn(game, 'checkWinCondition');

      await game.runGameLoop();

      // Verify CharacterGeneration phase was handled specially
      expect(charGenPhase.runStep).toHaveBeenCalledTimes(1);
      expect(checkWinSpy).not.toHaveBeenCalled(); // Should exit before win condition check
    });

    it('should enforce maximum iteration limit', async () => {
      // Setup: Mock a phase that never ends but makes small progress
      let iterationCount = 0;
      const endlessPhase = {
        type: 'Day' as GamePhaseType,
        runStep: vi.fn().mockImplementation(async () => {
          iterationCount++;
          // Simulate small progress to avoid infinite loop detection
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (game as any).setNextPlayerIndexToAction(iterationCount % 10);
        }),
        transition: vi.fn().mockReturnValue('Night'),
      };
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      // Mock the phase step to be 'Finished' to trigger transitions
      vi.spyOn(game, 'getPhaseStep').mockReturnValue('Finished');
    ...
    it('should handle phase transitions correctly', async () => {
      // Setup: Mock a sequence of phase transitions
      const dayPhase = {
        type: 'Day' as GamePhaseType,
        runStep: vi.fn().mockResolvedValue(undefined),
        transition: vi.fn().mockReturnValue('Night'),
      ...
      vi.spyOn(game, 'getPhaseStep')
        .mockReturnValueOnce('Start')
        .mockReturnValueOnce('Finished') // Day phase finished
        .mockReturnValueOnce('Start')
        .mockReturnValueOnce('Finished'); // Night phase finished

      // Mock advanceToPhase to actually change the phase
      advancePhaseSpy.mockImplementation((phaseType) => {
        if (phaseType === 'Night') {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (game as any)['#currentState'] = nightPhase;
        } else if (phaseType === 'GameOver') {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (game as any)['#currentState'] = { type: 'GameOver' };
        }
      });

      await game.runGameLoop();

      // Verify both phases ran and transitions occurred
      expect(dayPhase.runStep).toHaveBeenCalled();
      expect(nightPhase.runStep).toHaveBeenCalled();
      expect(advancePhaseSpy).toHaveBeenCalledWith('Night', undefined);
      expect(advancePhaseSpy).toHaveBeenCalledWith('GameOver', undefined);
    });
