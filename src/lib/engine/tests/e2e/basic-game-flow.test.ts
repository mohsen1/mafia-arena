...
describe('Basic Game Flow E2E', () => {
    ...
    beforeEach(() => {
        ...
        const playerSetups = [
            { name: 'Human Player', agent: new HumanAgent('human-1'), role: new VillagerRole() },
            { name: 'AI Mafia 1', agent: new MockAgent('ai-1'), role: new MafiaRole() },
            { name: 'AI Mafia 2', agent: new MockAgent('ai-2'), role: new MafiaRole() },
            { name: 'AI Doctor', agent: new MockAgent('ai-3'), role: new DoctorRole() },
    ...
    it('should handle Day phase without infinite loop', async () => {
        ...
        while (game.getPhaseStep() !== 'Finished' && stepCount < maxSteps) {
            ...
            console.log(`Day Phase Step: ${currentStep}, Players: ${game.getAlivePlayers().length}`);
            ...
            if (pendingAction) {
                ...
                if (humanPlayer) {
                    dayPhase.processAction(game, pendingAction.playerId, { type: 'noAction' });
    ...
    it('should handle Night phase without infinite loop', async () => {
        ...
        while (game.getPhaseStep() !== 'Finished' && stepCount < maxSteps) {
            ...
            console.log(`Night Phase Step: ${currentStep}, Players: ${game.getAlivePlayers().length}`);
    ...
    it('should detect win conditions correctly', () => {
        ...
        const mafiaPlayers = game.getAlivePlayers().filter(p => p.role.name === 'Mafia');
        ...
        const newGame = Game.createNewGame([
            { name: 'Human Player', agent: new HumanAgent('human-2'), role: new VillagerRole() },
            { name: 'AI Mafia 1', agent: new MockAgent('ai-5'), role: new MafiaRole() },
            { name: 'AI Mafia 2', agent: new MockAgent('ai-6'), role: new MafiaRole() },
        ], 'UK_VILLAGE_1900S', 'en');
        
        // Kill town player - Mafia should win (2 mafia vs 0 town)
        const townPlayers = newGame.getAlivePlayers().filter(p => p.role.allegiance === 'Town');
    ...
    it('should not get stuck in infinite loop with empty player actions', async () => {
        ...
        console.log(`Pending human action: ${pendingAction ? `${pendingAction.playerId} - ${pendingAction.allowedActions.join(', ')}` : 'None'}`);
        ...
        if (!pendingAction) {
            ...
            console.log('Game ended due to infinite loop protection - this is the expected fix');
        } else {
            console.log('Game paused for human action - this is also expected behavior');
        }
    });
});

// Helper function to complete a phase with human action simulation
async function completePhaseWithSimulation(game: Game, phaseType: string, maxSteps: number): Promise<void> {
    ...
    if (stepCount >= maxSteps) {
        throw new Error(`Phase ${phaseType} did not complete within ${maxSteps} steps`);
    }
} 