...
describe('InitializationPhase', () => {
  ...
  beforeEach(() => {
    ...
    mockPlayerObj1 = {
      ...
      name: 'Player 1',
      agent: mockAgent1,
      setName: vi.fn((newName: string) => {
        mockPlayerObj1.name = newName;
      }),
    };
    mockPlayerObj2 = {
      id: 'p2',
  ...
  it('should orchestrate persona generation and log appropriately', async () => {
    ...
    expect(mockLogMessage).not.toHaveBeenCalledWith(
      null,
      'Generating player personas...',
      MessageVisibility.Public,
      'Init'
    );

    expect(mockAgent1.generatePersona).toHaveBeenCalledTimes(1);
    expect(mockAgent1.generatePersona).toHaveBeenCalledWith(
      'A theme for testing',
      'en'
    );
    expect(mockAgent3.generatePersona).toHaveBeenCalledTimes(1);
    expect(mockAgent3.generatePersona).toHaveBeenCalledWith(
      'A theme for testing',
      'en'
    );
