...
describe('OpenAIAgent', () => {
  ...
  beforeEach(async () => {
    ...
    mockCreate = (mockedOpenAI as unknown as { __mockCreate: Mock })
      .__mockCreate; // Get the exported mock function
    MockOpenAIConstructor = (
      mockedOpenAI as unknown as { __MockOpenAIConstructor: Mock }
    ).__MockOpenAIConstructor; // Get constructor mock
    ...
    getPersonaGenerationPrompt =
      mockedPrompts.getPersonaGenerationPrompt as Mock; // Get the mock
  ...
  it('should call OpenAI API with configured model and prompts', async () => {
    ...
    expect(getUserPrompt).toHaveBeenCalledWith(
      expect.objectContaining({
        // Check the promptInputState structure
  ...
  it('should return noAction if the returned action type is not allowed', async () => {
    const disallowedAction: PlayerAction = {
      type: 'mafiaKill',
      targetPlayerId: 'p1',
    };
    mockCreate.mockResolvedValue({
    ...
    // Suppress console error for this test
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const action = await agent.getAction(mockGameState, [
      'message',
      'vote',
      'noAction',
    ]);
  ...
  it('should return noAction and log error on API call failure', async () => {
    ...
    expect(log.response.error).toContain(
      'API call failed: API failed miserably'
    );
    expect(log.response.parsedAction).toBeNull(); // No action parsed
  });
  ...
  it('should return noAction and log error on JSON parsing failure', async () => {
    const invalidJsonResponse =
      '{"type": "message", "content": "unterminated string';
    mockCreate.mockResolvedValue({
    ...
    expect(log.response.raw).toEqual(
      `[Response omitted - ${invalidJsonResponse.length} chars]`
    );
  ...
  it('should handle a Mafia player during the Night phase', async () => {
    ...
    const expectedAction: PlayerAction = {
      type: 'mafiaKill',
      targetPlayerId: villagerPlayerId,
    };
    mockCreate.mockResolvedValue({
      choices: [{ message: { content: JSON.stringify(expectedAction) } }],
    });
  ...
  it('should use existing memory when generating prompts', async () => {
    ...
    const memoryWithHistory: AgentMemory = {
      ...
      killHistory: [],
      saveHistory: [],
      voteHistory: [{ round: 1, votes: new Map([[player1Id, player2Id]]) }],
      messageHistory: [
        {
          id: 'msg1',
          round: 1,
          ...
          timestamp: new Date(),
          visibility: 'Public' as MessageVisibility, // Cast for simplicity
        },
      ],
    ...
    expect(getUserPrompt).toHaveBeenCalledWith(
      expect.objectContaining({
        memory: expect.objectContaining({
          voteHistory: expect.arrayContaining([
            // Check vote history structure
            expect.objectContaining({
              round: 1,
  ...
  describe('generatePersona', () => {
    ...
    it('should call API with persona prompt and update persona on success', async () => {
      ...
      mockCreate.mockResolvedValueOnce({
        choices: [
          { message: { content: JSON.stringify(mockGeneratedPersona) } },
        ],
      });
      ...
      expect(mockCreate).toHaveBeenCalledWith({
        model: testModel,
        messages: [{ role: 'user', content: 'Mock Persona Gen Prompt' }],
    ...
    it('should assign DEFAULT_PERSONA if API call fails', async () => {
      ...
      const consoleSpy = vi
        .spyOn(console, 'error')
        .mockImplementation(() => {}); // Suppress error
    ...
    it('should assign DEFAULT_PERSONA if parsed JSON lacks required fields', async () => {
      const incompletePersona = {
        name: 'Incomplete Guy',
        backstory: 'Missing traits.',
      }; // Missing personalityTraits
      mockCreate.mockResolvedValueOnce({
