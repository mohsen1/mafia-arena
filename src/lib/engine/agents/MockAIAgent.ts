    ...
    async getAction(gameState: VisibleGameState, allowedActions: PlayerAction['type'][] = []): Promise<PlayerAction> {
        ...
        if (gameState.phase === 'Day') {
            if (allowedActions.includes('message') && Math.random() < 0.5) {
                return { type: 'message', content: `${this.persona.name} suspects foul play.` };
            }
            ...
            switch (gameState.self.role) {
                case RoleName.Mafia:
                    if (allowedActions.includes('mafiaKill') && aliveOthers.length > 0) {
                        return { type: 'mafiaKill', targetPlayerId: getRandomElement(aliveOthers)! };
                    }
                    break;
                case RoleName.Doctor:
                    if (allowedActions.includes('doctorSave')) {
                        return { type: 'doctorSave', targetPlayerId: getRandomElement(aliveOthers) || null };
                    }
                    break;
                case RoleName.Seer:
                    if (allowedActions.includes('seerInvestigate')) {
                        return { type: 'seerInvestigate', targetPlayerId: getRandomElement(aliveOthers) || null };
                    }
                    break;
            }
            return { type: 'noAction' };
