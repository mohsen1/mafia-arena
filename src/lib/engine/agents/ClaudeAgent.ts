...
} from '../prompts'; // Import prompt functions
...
if (apiKey) {
  ...
  log(
    'WARN: ANTHROPIC_API_KEY environment variable not set. ClaudeAgent will not work.'
  );
}
...
export class ClaudeAgent implements IAgent {
  ...
  constructor(id: PlayerId, model?: string) {
    ...
    if (!anthropic) {
      log(
        `ERROR: ClaudeAgent ${this.id}: Anthropic client not available. Agent cannot function.`
      );
  ...
  async generatePersona(
    ...
    if (!anthropic) {
      log(
        `ERROR: [${agentIdForLog}] Anthropic client not initialized. Using default persona.`
      );
    ...
    try {
      const response = await anthropic.messages.create({
        ...
        system:
          'You are a creative writer. Respond ONLY with the requested JSON object.', // Simpler system prompt for JSON
        messages: [
  ...
  async getAction(
    ...
    if (!anthropic) {
      log(
        `ERROR: ClaudeAgent ${this.id}: Anthropic client not initialized. Returning noAction.`
      );
    ...
    try {
      ...
      try {
        ...
        log(
          `ERROR: [${agentIdForLog} (Claude)] Failed to parse JSON response: ${potentialJson} %O`,
          parseError
        );
        return { type: 'noAction' };
      }
      ...
      if (
        ...
        log(
          `WARN: [${agentIdForLog} (Claude)] Action type '${action.type}' is not in allowed actions: ${allowedActions.join(', ')}. Defaulting to noAction.`
        );
        return { type: 'noAction' };
      }
      if (!allowedActions && action.type !== 'noAction') {
        log(
          `WARN: [${agentIdForLog} (Claude)] Action type '${action.type}' is not allowed (no actions specified). Defaulting to noAction.`
        );
        return { type: 'noAction' };
      }

      log(
        `[${agentIdForLog} - ${gameState.self.role} (Claude)] Chose action: %o`,
        action
      );
      return action;
    } catch (error) {
      log(
        `ERROR: [${agentIdForLog} (Claude)] Error calling Anthropic API: %O`,
        error
      );
      return { type: 'noAction' };
    }
