# Werewolf AI Documentation

This directory contains comprehensive documentation for the Werewolf AI project. Below you'll find guides for deployment, configuration, and advanced features.

## Table of Contents

### 🚀 Deployment & Setup
- [Vercel Deployment Guide](./VERCEL_DEPLOYMENT.md) - Step-by-step guide for deploying to Vercel
- [Vercel Environment Checklist](./VERCEL_ENV_CHECKLIST.md) - Complete list of environment variables for Vercel
- [Database Migrations on Vercel](./DATABASE_MIGRATIONS_VERCEL.md) - How database migrations work in production
- [Database Migration Conventions](./DATABASE_MIGRATION_CONVENTIONS.md) - Best practices for creating migrations
- [Database Index Strategy](./DATABASE_INDEXES.md) - Database performance optimization guide

### 🤖 AI Configuration
- [Ollama Integration](./OLLAMA_INTEGRATION.md) - Run AI models locally with Ollama
- [Ollama Local Setup](./OLLAMA_LOCAL_SETUP.md) - Detailed Ollama installation and configuration
- [User API Keys](./USER_API_KEYS.md) - How users can bring their own API keys
- [Enable OpenAI Models](./ENABLE_OPENAI_MODELS.md) - OpenAI integration guide

### 🧪 Testing & Development
- [Error Handling Test Plan](./ERROR_HANDLING_TEST_PLAN.md) - Comprehensive testing strategies
- [Security Practices](./SECURITY_PRACTICES.md) - Security measures and best practices

### 📚 Additional Resources
- [Main README](../README.md) - Project overview and quick start
- [Architecture Guide](../ARCHITECTURE.md) - Technical architecture deep dive
- [Contributing Guide](../CONTRIBUTING.md) - How to contribute to the project

## Quick Links

- **Getting Started**: See the [main README](../README.md)
- **Local Development**: Use Docker for database setup (see main README)
- **Production Deployment**: Follow the [Vercel Deployment Guide](./VERCEL_DEPLOYMENT.md)
- **AI Providers**: Configure [Ollama](./OLLAMA_INTEGRATION.md) or [User API Keys](./USER_API_KEYS.md)

## Need Help?

- Check the relevant guide above
- Review the [Architecture Guide](../ARCHITECTURE.md) for technical details
- Open an issue on GitHub for bugs or feature requests 