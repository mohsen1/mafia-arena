...
-- Note: All DROP operations use IF EXISTS to handle missing indexes gracefully
...
-- Add comments explaining index strategy (only if indexes exist)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_games_owner_status') THEN
        COMMENT ON INDEX "idx_games_owner_status" IS 'Composite index for filtering games by owner and status';
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_game_participants_game_user') THEN
        COMMENT ON INDEX "idx_game_participants_game_user" IS 'Composite index for participant lookups';
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_api_keys_user_provider') THEN
        COMMENT ON INDEX "idx_user_api_keys_user_provider" IS 'Composite index for API key lookups';
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_achievements_user_achievement') THEN
